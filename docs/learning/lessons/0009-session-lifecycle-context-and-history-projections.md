...
| 问题 | 压缩后的默认答案 |
| --- | --- |
| 原始内容是否仍被系统保存？ | 通常是，仍在 rollout/Thread Store 中 |
| 原始内容是否仍在模型当前 Prompt 中？ | 不一定，只有 replacement history 保留的部分仍在 |
| 模型是否有标准工具主动回读原始 Thread 历史？ | 普通 Session 默认没有 |

假设 checkpoint 以前有一条精确要求：

```text
用户：超时值必须改成 37 秒，不要使用 30 秒。
```
...
- 界面仍然显示“37 秒”；
- rollout 仍然保存“37 秒”；
- replacement history 只保留“调整超时配置”；
- 新模型请求中没有“37 秒”；
- 模型无法因为界面还能滚动，就自动读取那一行。

所以压缩不是把旧内容移动到模型随时可查的隐藏区，而是一次可能有信息损失的上下文重写。

### `thread/read` 为什么不能解决默认模型回忆？

App Server 提供 `thread/read`、`thread/turns/list` 和 `thread/items/list` 等客户端 API。TUI 或其他客户端可以用它们读取存储历史。

但这类 API 位于客户端到 App Server 的协议边界，不会因为存在就自动成为模型工具。普通 Task 构造 `ToolRouter` 时，不会把 `thread/read` 注册给模型。
...
客户端当然可以先读取旧历史，再把相关内容作为新输入显式注入 Turn；但这是宿主协调，不是模型自身的默认回忆能力。

### 显式例外：history-notes extension

当前源码包含一个可选的 `history-notes` extension。满足配置和后端条件时，它可以向模型暴露只供模型使用的历史工具：
...
这些工具允许模型列出历史窗口、搜索内容和按范围读取旧项目。因此在启用该扩展的产品形态中，模型可能重新取得 checkpoint 以前的原始信息。

但它的工作方式仍然不是“旧内容一直留在 Prompt 中”，而是：
...
这项能力不能当作普通 Session 的普遍保证，因为：

- `use_history_notes_extension` 默认是 `false`；
- 它要求相应的 OpenAI/Codex 后端条件；
- 模型必须意识到需要检索，并给出可搜索的线索；
- 检索结果仍受范围、字符数和工具输出限制。

Guardian reviewer 还有一条特殊路径：如果父 Session 已有这些 history tools，可以把其中的只读工具交给 reviewer。它同样属于专门模式，不改变普通 Agent 的默认结论。
...
- 用户重新发送旧要求；
- 客户端读取 Thread 历史并把相关内容注入新 Turn；
- 重要结论已经写入仓库文件、计划或外部状态存储，模型重新读取；
- 重新执行工具，再次产生所需结果；
- Fork 或 Revert 到另一条保留相应上下文的历史路径。

这些都不是“模型天然记得压缩前内容”，而是把外部事实重新导入模型的可操作范围。

因此应记住一句判断公式：

> **系统保存了什么、模型此刻直接知道什么、模型还能按需检索什么，是三个不同问题。**

这也是为什么长任务中的精确约束、关键决策和实施进度不能只依赖聊天历史。重要状态应当外置到文件、计划、数据库或专门的 memory/history 服务中。

## 十三、与 Harness“状态外置”的关系
...
11. 默认模型没有主动回读 checkpoint 前 rollout 的工具；界面可见不代表模型可见。
12. 显式启用 history-notes extension 后，模型可以通过 history 工具按需检索旧窗口，但这不是默认保证。
