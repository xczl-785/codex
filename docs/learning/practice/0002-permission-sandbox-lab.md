...
| 观察对象 | 必须回答的问题 | 应保存的证据 |
| --- | --- | --- |
| 事件顺序 | 谁拒绝的？审批发生了吗？进程启动了吗？结果是否已经返回调用方？ | 有序事件及 CallId、ApprovalId、AttemptId、模拟或真实 ProcessId；标出绝不能出现的事件 |
| 有效权限 | 默认范围是什么？本次批准扩大到哪里？哪些限制仍保留？下一次能否复用批准？ | 默认 Profile、本次请求、批准范围、有效执行权限、授权复用范围与缓存/规则命中结果 |
| 执行前后状态 | 哪些修改已经生效？有没有不完整目标文件？重试会不会重复或覆盖？ | 前后文件内容或哈希、日志追加次数、产物核验结果，以及下一动作的依据 |

在 `docs/EXPERIMENTS.md` 中，每个场景固定记录“前置状态 → 预期事件/权限/文件结果 → 实际证据 → 差异解释 → 下一动作”。Fake 阶段明确标注模拟事件与内存文件状态，真实阶段再记录系统执行证据。

优先完整走通报告案例：追加日志 → 生成报告 → 越界复制失败 → 核验报告 → 获批后只复制。重点解释为什么失败后源报告仍存在、为什么日志只追加一次、为什么本次批准不自动授权下一次调用，而不只记录测试通过。
